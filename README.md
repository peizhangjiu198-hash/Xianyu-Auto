...
│   ├── main.ts              # 主进程入口
│   ├── preload.ts           # 预加载脚本
│   ├── device_id_generator.py  # 设备ID生成
│   └── status_poller.py     # 状态轮询服务
├── frontend/                # 前端 Vue 应用
│   ├── src/
│   │   ├── views/           # 页面组件
│   │   │   ├── dashboard/   # 控制台
...
所有配置项统一在 `backend/config.py` 中管理，严禁硬编码。

| 配置项 | 说明 |
|--------|------|
| `BACKEND_PORT` | 后端服务端口（默认 9000） |
| `DATA_DIR` | 数据目录路径 |
| `LOG_DIR` | 日志目录路径 |
...
程序启动后自动生成设备唯一标识，并每 15 秒轮询服务器验证激活状态。

- **服务器地址**：`https://kami.pecj.xyz`
- **接口路径**：`POST /api/public/status-by-pc`
- **请求参数**：`{ "pc_machine_id": "XY-xxxxx" }`
...
| `/api/ai/reply` | POST | AI 智能回复 |
| `/api/keywords` | CRUD | 关键词管理 |

### WebSocket

| 事件 | 方向 | 说明 |
|------|------|------|
| `message` | Server → Client | 新消息通知 |
| `status` | Server → Client | 连接状态 |
...
1. **配置统一管理**：所有配置项在 `backend/config.py` 中管理，禁止硬编码
2. **路径日志**：所有 JSON 文件的读取、加载、保存操作必须有完整路径日志
3. **字段隔离**：`itemPic` 和 `itemTitle` 只从会话列表源数据获取，不回退到会话消息
4. **布局规范**：左侧栏固定 320px，右侧聊天区高度自适应
5. **代码去重**：公共函数统一放在对应模块，禁止重复定义
